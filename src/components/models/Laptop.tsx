...
export function Laptop() {
  return (
    <InteractiveProject projectId="cypress">
      {(hovered) => (
        <group position={[-0.45, 0.78, -1.28]}>
          <mesh castShadow receiveShadow position={[0, 0.03, 0.08]}>
            <boxGeometry args={[0.8, 0.05, 0.52]} />
            <meshStandardMaterial color="#2b2f33" roughness={0.45} />
          </mesh>
          <mesh castShadow receiveShadow position={[0, 0.35, -0.18]} rotation={[-0.35, 0, 0]}>
            <boxGeometry args={[0.82, 0.52, 0.05]} />
            <meshStandardMaterial color="#25282c" roughness={0.4} />
          </mesh>
          <mesh castShadow receiveShadow position={[0, 0.35, -0.21]} rotation={[-0.35, 0, 0]}>
